...
pub trait PackageManagerBackend: Send + Sync {
    ...
    /// Detect and fix broken package dependencies before patch operations.
    /// Returns Ok(true) if broken dependencies were found and fixed, Ok(false) if none found.
    /// Default implementation is a no-op for backends that don't support this concept.
    fn detect_and_fix_broken_dependencies(&self) -> Result<bool> {
        Ok(false)
    }
}
...
impl AptBackend {
    ...
    /// Detect broken dpkg state by running dpkg --audit.
    /// Returns true if any broken/unconfigured packages are found.
    fn detect_broken_dependencies(&self) -> Result<bool> {
        let output = self.run_dpkg(&["--audit"])?;
        // dpkg --audit outputs nothing if everything is fine.
        // Any non-empty output indicates broken/unconfigured packages.
        Ok(!output.trim().is_empty())
    }

    /// Fix broken dependencies by running apt --fix-broken install.
    fn fix_broken_dependencies(&self) -> Result<()> {
        info!("Running apt --fix-broken install to correct broken dependencies");
        self.run_apt(&["--fix-broken", "install", "-y"])?;
        info!("Broken dependencies fix command completed");
        Ok(())
    }
...
impl PackageManagerBackend for AptBackend {
    ...
    fn detect_and_fix_broken_dependencies(&self) -> Result<bool> {
        if !self.detect_broken_dependencies()? {
            return Ok(false);
        }

        tracing::warn!("Broken dependencies detected -- running automatic repair");
        self.fix_broken_dependencies()?;

        // Verify the fix succeeded
        if self.detect_broken_dependencies()? {
            return Err(anyhow::anyhow!(
                "Broken dependencies persist after apt --fix-broken install"
            ));
        }
