...
pub fn spawn_crl_refresh_task(
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let result = refresh_crl(&manager_url, &crl_path, &ca_cert_der, &shared_state).await;
            match result {
                Ok(()) => info!("CRL background refresh completed successfully"),
                Err(e) => warn!(
                    error = %e,
