        ...
        .await
    {
        Ok(job_id) => {
            ...
            tokio::spawn(async move {
                ...
                // Check for and fix broken dependencies before applying patches
                let _ = job_manager_clone
                    .update_job(
                        &job_id_clone,
                        JobStatus::Running,
                        Some(15),
                        Some("Checking for broken dependencies...".to_string()),
                    )
                    .await;
                let _ = job_manager_clone
                    .add_job_log(&job_id_clone, "Checking for broken dependencies...".to_string())
                    .await;

                match backend_clone.detect_and_fix_broken_dependencies() {
                    Ok(true) => {
                        let _ = job_manager_clone
                            .add_job_log(
                                &job_id_clone,
                                "Broken dependencies detected and fixed automatically".to_string(),
                            )
                            .await;
                    }
                    Ok(false) => {
                        let _ = job_manager_clone
                            .add_job_log(&job_id_clone, "No broken dependencies found".to_string())
                            .await;
                    }
                    Err(e) => {
                        let err_msg = format!("Failed to fix broken dependencies: {}", e);
                        error!(job_id = %job_id_clone, error = %e, "Broken dependency fix failed");
                        let _ = job_manager_clone
                            .add_job_log(&job_id_clone, err_msg.clone())
                            .await;
                        let _ = job_manager_clone.fail_job(&job_id_clone, err_msg).await;
                ...
                match apply_result {
                    ...
                    Err(e) if crate::packages::cache::is_fetch_error(&e) => {
                        ...
                        // Re-check broken dependencies after cache refresh on retry
                        match backend_clone.detect_and_fix_broken_dependencies() {
                            Ok(true) => {
                                let _ = job_manager_clone
                                    .add_job_log(
                                        &job_id_clone,
                                        "Broken dependencies detected and fixed during retry".to_string(),
                                    )
                                    .await;
                            }
                            Ok(false) => {}
                            Err(e) => {
                                let err_msg = format!("Failed to fix broken dependencies during retry: {}", e);
                                let _ = job_manager_clone.fail_job(&job_id_clone, err_msg.clone()).await;
                                error!(job_id = %job_id_clone, error = %e, "Broken dependency fix failed during retry");
                                return;
                            }
                        }
