...
4. **SIGTERM handler must exit immediately during mutations:** The SIGTERM handler must NOT wait for an in-progress package mutation to "complete" before exiting. Whenever the agent's own package is being upgraded (explicit self-update OR patch_apply via dist-upgrade), the postinst fires `systemctl restart --no-block`, which sends SIGTERM to the agent. If the handler waits for the mutation, it deadlocks: the mutation waits for dpkg → dpkg waits for postinst → postinst waits for systemctl restart → systemd waits for the agent to exit. The fix: exit immediately on SIGTERM whenever any mutation is in progress — process-group isolation protects the transaction. The old 100s wait loop was a pre-isolation safety measure that is now obsolete.
5. **Job persistence must distinguish self-update restarts from crashes:** PR #200's orphaned-job recovery marked ALL orphaned jobs as Failed/AGENT_REBOOTED. But a self-update job is orphaned *because* the postinst restart succeeded — the new binary is running, which proves the upgrade worked. SelfUpdate orphans must be marked Completed; only genuine crashes (other operations) are marked Failed.
