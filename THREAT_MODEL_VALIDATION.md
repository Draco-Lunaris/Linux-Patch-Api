...
| Arbitrary file write via staging | Staging dir root-owned, mode 0700 | SPEC.md: file_staging_dir permissions enforced | ✅ Mitigated | Medium |
| GPG signing bypass | Config gate: allow_file_install defaults to false | SPEC.md: Must be explicitly enabled; documented security risk | ✅ Mitigated | High |
...
**File Install Assessment:** The file install feature introduces a controlled risk by bypassing repository GPG signing. This is mitigated through defense-in-depth: the feature is disabled by default (`allow_file_install: false`), extension allowlisting prevents arbitrary file uploads, size limits prevent disk exhaustion, and staging directory permissions prevent tampering. The `--allow-untrusted` flag for APK is an acceptable risk since the entire file install feature is opt-in and the user explicitly chooses to install an untrusted file.

**Evidence Sources:**
- SPEC.md: File Install Endpoint section, Security section

**Note:** File installs inherently bypass repository GPG verification. Administrators must understand this trade-off when enabling `allow_file_install`. Only use with files from trusted sources.
